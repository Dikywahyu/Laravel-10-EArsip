                            ...
                            <div class="modal-body">
                                <form class="form-horizontal mt-4 row" id="formdata">
                                    ...
                                    <div class="form-group col-12">
                                        <label class="form-label">Nama Pencipta </label>
                                        <input type="text" class="form-control" id="nama_pencipta_arsips">
                                    </div>
                ...
                <script>
                        ...
                        aoColumns: [
                            ...
                            }, 
                            {
                                ...
                                "render": function(data, row, type, meta) {
                                    return '<button   title="Edit Data"  id="EditData" value="' + data.pencipta_arsips_id + '" class="btn btn-primary waves-effect waves-light"><i class="fas fa-edit "></i></button> ' +
                                        ' <button title="Hapus Data"  id="Destroy" value="' + data.pencipta_arsips_id + '" class="btn btn-danger waves-effect waves-light"><i class="fas fa-trash"></i></button>'+
                                        ' <a href="unitpencipta/' + data.pencipta_arsips_id + '" target="_blank" class="btn btn-success waves-effect waves-light"><i class="fas fa-plus"></i> Unit</a>';
                    ...
                    $('#store').click(function(e) {
                        ...
                        if ($('#id_conten').val() == "") {
                            ...
                            var fd = new FormData();
                            fd.append('nama_pencipta_arsips', $('#nama_pencipta_arsips').val());  
                            fd.append('_token', CSRF_TOKEN);

                            $.ajax({
                                url: "{{ route('pencipataarsip.store') }}",
                                method: 'post',
                                data: fd,
                                contentType: false,
                                processData: false,
                            ...
                            $.ajax({
                                url: "{{ route('pencipataarsip.update') }}",
                                method: 'post',
                                data: fd,
                                contentType: false,
                                processData: false,
                    ...
                    $('#datatable-main').on('click', '#EditData', function() {
                        ...
                        Swal.fire({
                            ...
                            if (result.value) {
                                $.ajax({
                                    method: 'post',
                                    url: "{{route('pencipataarsip.show','src')}}",
                                    cache: false,
                                    data: {
                                        "post_data": post_data,
                                        "_token": token
                                    ...
                                    success: function(response) {
                                        $("#MTData").modal('show');
                                        $("#id_conten").val(response.data.pencipta_arsips_id).change(); 
                                        $("#nama_pencipta_arsips").val(response.data.nama_pencipta_arsips).change();  
                                    },
                                    error: function(response) {
                    ...
                    $('#datatable-main').on('click', '#Destroy', function() {
                        ...
                        Swal.fire({
                            ...
                            if (result.value) {
                                $.ajax({
                                    method: 'delete',
                                    url: "{{ route('pencipataarsip.destroy') }}",
                                    cache: false,
                                    data: {
                                        "post_data": post_data,
                                        "_token": token
